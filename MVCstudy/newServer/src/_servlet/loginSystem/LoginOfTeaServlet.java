...
public class LoginOfTeaServlet extends HttpServlet {
		...
		try {
			...
			if(password == null) {
				...
				if(password.equals(lTeaPassword) ) {
					...
					resp.sendRedirect("http://localhost:8080/newServer/index.jsp");
				}
				else {
	...
	//具体连接mybatis方法
	private String getTeacher(String teaID_tmp) {
		SqlSession sqlSession = sqlSessionFactory.openSession();
		TeacherDAO teacherDAO = sqlSession.getMapper(TeacherDAO.class);
		Teacher teacher = teacherDAO.getTeacherById(teaID_tmp);
		//提交
		sqlSession.commit();
		if(teacher == null) {
			return null;
